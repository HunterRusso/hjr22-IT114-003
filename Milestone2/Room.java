...
public class Room implements AutoCloseable{
	...
	// Remove the 'static' keyword from the methods
	public synchronized void setCorrectOption(String correctOption) {
	...
	public void sendPayloadToClients(Payload payload) {
		for (ServerThread client : clients) {
			client.send(payload);
		}
	}
	...
	private void sendPlayerScore(ServerThread client) {
		int playerScore = client.getScore();
		client.sendMessage("Server", "Your current score: " + playerScore);
	}
	...
	protected synchronized void sendMessage(ServerThread sender, String message) {
		...
		// Include the correct option in the message
		String messageToSend = message;
	
		Iterator<ServerThread> iter = clients.iterator();
		while (iter.hasNext()) {
			ServerThread client = iter.next();
			boolean messageSent = client.sendMessage(from, messageToSend);
			if (!messageSent) {
	...
	void startRound() { // starts a new round
		server.startRound(this);
	}
    protected static void switchRoom(ServerThread client, Room oldRoom, Room newRoom) {
        if (oldRoom != null) {
