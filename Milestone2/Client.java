...
public class Client {
    ...
    private void sendReadyPayload() throws IOException {
        ...
        p.setPlayerIds(Collections.singletonList(clientName));
        out.writeObject(p);
    }
    ...
    private boolean processCommand(String text) throws IOException {
        if (isConnection(text)) {
            ...
            sendReadyPayload();
            return true;
        } else if (isPickCommand(text)) {
            // Handle pick command
            String option = extractOptionFromCommand(text);
            sendPickPayload(option);
            return true;
        } else if (isPassCommand(text)) {
            // Handle pass command
            sendPassPayload();
            return true;
        }
                            ...
                            if (!processCommand(line)) {
                                if (isConnected()) {
                                    if (line != null && line.trim().length() > 0) {
                                        ...
                                        if (line.trim().equalsIgnoreCase("/ready")) {
                                            sendReadyPayload();
                                        } else if (line.trim().startsWith("/pick")) {
                                            // Parse the option from the user input
                                            String option = line.trim().substring(6).trim();
                                            sendPickPayload(option);
                                        } else if (line.trim().equalsIgnoreCase("/pass")) {
                                            sendPassPayload();
                                        } else {
                                            sendMessage(line);
                                        }
